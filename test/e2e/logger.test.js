...
const failLogger = errorLogger(_.assign(_.cloneDeep(config), { 'AUTH0_CLIENT_ID': 'invalid' }))
...
describe('Logger E2E Tests', () => {
  ...
  describe('error method test', () => {
    ...
    it('error method invoked by fail logger, log an error', async () => {
      try {
        await failLogger.error(new Error('This is an error'))
  ...
  describe('logFullError method test', () => {
    ...
    it('logFullError method invoked by fail logger, log an error', async () => {
      try {
        await failLogger.logFullError(new Error('This is an error'))
        throw new Error('should not throw error here')
      } catch (err) {
        should.equal(err.message, 'Unknown Error')
      }
